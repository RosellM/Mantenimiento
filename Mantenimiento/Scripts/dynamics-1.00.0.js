...
(function ($) {
    $.fn.dynamicTable = function (options) {
        ...
        $("#" + $(id).parent().parent()[0].id + "Titulo a").click();
        //Se agregan los botonoes a la lista
        var html = "<div class='col-lg-12'><div class='btn-group pull-right' role='group'>";
        $(settings.data.iconosGrid).each(function (key, value) {
            html += " <button type='button' class='btn btn-info' onclick='" + String(value["callback"]) + "($(this))' ";
            if (!value["enabled"])
                html += " disabled "
            html += " data-toggle='tooltip' data-placement='bottom' title='" + value["tooltip"] + "' ";
            html += " tipoCRUD= '" + value["tipoCRUD"] + "' " + "defaultEnabled=" + value["enabled"];
